...
function QuestaoDetalhes() {
    ...
    const [questoesPorPagina, setQuestoesPorPagina] = useState(1);
    ...
    //     });
    // }, []);
    const { questionId } = useParams(); // Obtém o questionId da URL
    const [question, setQuestion] = useState(null);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        console.log("questionId da URL:", questionId);
        ...
        onValue(questionsRef, (snapshot) => {
            ...
            console.log("Dados das questões:", questionArray);

            const selectedQuestion = questionArray.find((q) => q.ids.toString() === String(questionId));
            ...
            if (selectedQuestion) {
                ...
                console.error("Questão com o id fornecido não foi encontrada.");
                setQuestion(null); // Caso não encontre
            }
            setLoading(false);
        });
    }, [questionId]);
    ...
    if (!question) {
        return <Typography>Questão não encontrada.</Typography>;
    }

    return (
        <>
            <Header />
            <Container maxWidth="xl" sx={{ padding: '0em', maxWidth: 'true' }} className="fundo-Home">
                <Container maxWidth='xl'>
                    <Typography sx={{ fontSize: { xs: '0.8em', md: '1.2em' }, fontWeight: '400', fontFamily: 'Poppins', textAlign: "center", padding: '1em', paddingTop: '0.400em', color: "#1c5253" }}>
                        A única plataforma de Questões de concursos especializada em Serviço Social
                    </Typography>
                    <Box sx={{ display: "flex", justifyContent: "right", marginBottom: "1em" }}>
                        <Button
                        ...
                        >
                            a<span style={{ fontSize: `${fontSize}em`, fontWeight: "normal" }}>-</span>
                        </Button>
                    </Box>
                ...
                }} border="1px solid #ccc" borderRadius={1} padding={2} ref={componentRef}>
                    <Cronometro />
                </Box >

                <Box component={Paper} id="pdfContent">
                    <Box
                        sx={{
                    ...
                    <Box sx={{ borderLeft: "1px solid #1c52531e", borderRight: "1px solid #1c52531e", borderBottom: "1px solid #1c52531e" }}>
                        ...
                        <p className="enunciado" dangerouslySetInnerHTML={{ __html: question.enunciado }} style={{
                            fontSize: `${fontSize}em`,
                            fontWeight: "normal",
                            fontFamily: "Poppins",
                            textAlign: "left",
